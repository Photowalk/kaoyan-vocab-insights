...
import { Search, TrendingUp, Award, Clock, BookOpen, ChevronLeft } from 'lucide-react';
...
export default function KaoyanDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#FDFDFD] text-[#002147] selection:bg-[#D4AF37]/20">
      ...
      <div className="relative max-w-[1600px] mx-auto px-4 sm:px-6 py-6 md:px-12 md:py-12">
        {/* Navbar-style Header */}
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 mb-10 md:mb-16 border-b border-slate-100 pb-8 md:pb-12">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div className="flex items-center gap-3 text-slate-400">
              ...
              <p className="text-xs md:text-sm font-light tracking-[0.1em] uppercase">1997 – 2025 Comprehensive Analysis</p>
            </div>
          </motion.div>
          ...
          <div className="flex items-center gap-6 md:gap-12">
            <div className="hidden sm:block">
              ...
              <p className="text-2xl md:text-3xl font-serif font-bold italic">1.2k+</p>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-start">
          {/* Left: Interactive Navigation - Hidden on mobile if detail is open */}
          <div className={`lg:col-span-4 space-y-6 md:space-y-8 sticky top-4 md:top-8 ${isMobileDetailOpen ? 'hidden lg:block' : 'block'}`}>
            <div className="space-y-4">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-3 md:py-4 bg-white border border-slate-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-[#D4AF37]/5 focus:border-[#D4AF37]/30 transition-all shadow-sm text-sm"
                  value={searchTerm}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {allTags.map(tag => (
                  <button 
                    ...
                    className={`px-3 py-1.5 md:px-4 md:py-1.5 rounded-full text-[10px] md:text-[11px] font-bold tracking-wider uppercase transition-all ${selectedTag === tag ? 'bg-[#D4AF37] text-white shadow-lg shadow-amber-600/20' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}
                  >
                    {tag}
                  </button>
                ))}
            ...
            <div className="h-[calc(100vh-280px)] md:h-[calc(100vh-350px)] overflow-y-auto pr-2 md:pr-3 space-y-2 md:space-y-3 custom-scrollbar">
              <AnimatePresence mode="popLayout">
                {filteredData.map((item) => (
                  <motion.div
                    ...
                    className={`p-4 md:p-5 rounded-2xl cursor-pointer transition-all duration-300 flex items-center justify-between group ${selectedWord?.word === item.word ? 'bg-white shadow-xl md:shadow-2xl shadow-slate-200 border border-[#D4AF37]/20 ring-1 ring-[#D4AF37]/10' : 'hover:bg-white hover:shadow-xl hover:shadow-slate-100 border border-transparent'}`}
                  >
          ...
          {/* Right: Deep Analytics View - Shown conditionally on mobile */}
          <div className={`lg:col-span-8 ${isMobileDetailOpen ? 'block' : 'hidden lg:block'}`}>
              ...
              {selectedWord ? (
                ...
                >
                  ...
                  <button 
                    ...
                    className="lg:hidden mb-8 flex items-center gap-2 text-slate-500 hover:text-[#002147] transition-colors"
                  >
                    <ChevronLeft className="w-5 h-5" />
                    <span className="font-bold uppercase tracking-wider text-[11px]">Back to Lexicon</span>
                  </button>

                  <div className="flex flex-col xl:flex-row justify-between items-start mb-10 md:mb-16 gap-6 md:gap-8">
                    <div className="space-y-4 md:space-y-6">
                      <div className="inline-flex items-center gap-2 md:gap-3 px-3 md:px-4 py-1.5 md:py-2 bg-[#D4AF37]/5 text-[#D4AF37] rounded-xl border border-[#D4AF37]/10">
                  ...
                  <div className="mb-10 md:mb-16">
                    ...
                    <div className="h-[250px] md:h-[400px] w-full -ml-4 md:-ml-8">
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 md:gap-8">
                    <div className="group p-6 md:p-8 bg-[#002147]/[0.02] rounded-2xl md:rounded-[2rem] border border-slate-100 hover:border-[#D4AF37]/20 transition-colors">
                    ...
                    <div className="group p-6 md:p-8 bg-[#002147]/[0.02] rounded-2xl md:rounded-[2rem] border border-slate-100 hover:border-[#D4AF37]/20 transition-colors">
                      <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-[#D4AF37] text-white flex items-center justify-center text-[10px] md:text-xs">02</div>
